...
class gitcvs extends Controller {
	...
	public function updateRepo($query, $id = ''){

		if(!(REQUIRE_GIT_TRACKING)) {

			$this->absoluteRedirect(BASE_URL . 'profile/v/' . $id);
			return;
		}
		...
		if($files['A']){ 
				...
				array_push($statusMsg, ' Addition of JSON for Fellows is completed');
		}	
		if($files['M']){ 
				$this->model->gitProcess($repo, $files['M'], 'add', GIT_MOD_MSG, $user);
				array_push($statusMsg, ' Modification of JSON for Fellows is completed');
		}		
		if($files['D']){ 
				$this->model->gitProcess($repo, $files['D'], 'rm', GIT_DEL_MSG, $user);
				array_push($statusMsg, ' Deleted of JSON for Fellows is completed');
		}	
